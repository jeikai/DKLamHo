...
<script>
  const renderChart = (chart, arr) => {
    ...
    const titles  = ['Lượt xe đăng kiểm theo tháng', 'Lượt xe đăng kiểm theo năm', 'Lượt xe sắp hết hạn', 'Dự báo đăng kiểm trong năm']
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        for (let j = 0; j < 12; j++) {
          if (a.getMonth() == j) {
            ++registerByMonth[j];
            forecast[j] += Math.floor(registerByMonth[j]*j/10);
          }
