...
if (!isset($_GET['edit'])) {
    echo "<script>
    location.href = ' ../../component/home/home.php';
    </script>";
}
$edit_id = $_GET['edit'];
    ...
    <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" action="<?php echo $_SERVER['PHP_SELF'] . '?edit=' . $edit_id; ?>" method="post">
        ...
        ?>
            ...
            <div class="mb-4">
                ...
                <input class="bg-gray-100 text-gray-700 border-2 border-gray-200 rounded w-full py-2 px-4 leading-tight" type="text" value="<?php echo $row['number_car']; ?>" disabled>
            </div>
            <!-- Nhãn hiệu -->
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Nhãn hiệu:</label>
                <input class="bg-gray-100 text-gray-700 border-2 border-gray-200 rounded w-full py-2 px-4 leading-tight" type="text" value="<?php echo $row['brand']; ?>" disabled>
            </div>
            <!-- Chủ sở hữu -->
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Chủ sở hữu:</label>
                <input class="bg-gray-100 text-gray-700 border-2 border-gray-200 rounded w-full py-2 px-4 leading-tight" type="text" value="<?php echo $row['driverName']; ?>" disabled>
            </div>
            <!-- Số điện thoại -->
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Số điện thoại:</label>
                <input class="bg-gray-100 text-gray-700 border-2 border-gray-200 rounded w-full py-2 px-4 leading-tight" type="text" value="<?php echo $row['phoneNumber']; ?>" disabled>
            </div>
            <!-- Ngày đăng kiểm -->
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Ngày đăng kiểm:</label>
                <input class="bg-white text-gray-700 border-2 border-gray-200 rounded w-full py-2 px-4 leading-tight" name="registerDate" type="date" value="<?php echo $row['registerDate']; ?>">
            </div>
            <!-- Ngày hết hạn -->
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Ngày hết hạn:</label>
                <input class="bg-white text-gray-700 border-2 border-gray-200 rounded w-full py-2 px-4 leading-tight" name="expireDate" type="date" value="<?php echo $row['expireDate']; ?>">
            </div>
            <!-- Button -->
